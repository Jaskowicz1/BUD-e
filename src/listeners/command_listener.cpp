...
dpp::task<void> command_listener::on_slashcommand(dpp::slashcommand_t event) {
	for(auto& cmd : BUDe::commands) {
		if(cmd->command_name == event.command.get_command_name()) {
			...
			if(cmd->is_private() && event.command.usr.id.str() != "447098177879932939") {
